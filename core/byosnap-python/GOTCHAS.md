...
```python
@app.route("/v1/byosnap-core/users/<user_id>/game", methods=["GET"])
```
- All externally accessible APIs need to start with /$prefix/$byosnapId/remaining_path. where $prefix = v1, $byosnapId = byosnap-core and remaining_path = /users/<user_id>.
```python
@app.route("/v1/byosnap-core/users/<user_id>/game", methods=["GET"])
```
...
app.add_url_rule('/v1/byosnap-core/users/<user_id>/game',
