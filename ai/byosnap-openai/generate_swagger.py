...
app.add_url_rule('/v1/byosnap-openai/chat',
                 view_func=chat, methods=['POST'])
app.add_url_rule('/v1/byosnap-openai/chat-stream',
                 view_func=chat_stream, methods=['POST'])
app.add_url_rule('/v1/byosnap-openai/completion',
                 view_func=completion, methods=['POST'])
app.add_url_rule('/v1/byosnap-openai/completion-stream',
                 view_func=completion_stream, methods=['POST'])
app.add_url_rule('/v1/byosnap-openai/embedding',
                 view_func=embedding, methods=['POST'])

# Initialize APISpec
spec = APISpec(
    title="BYOSnap OpenAi",
    version="1.0.0",
...
with app.test_request_context():
    ...
    spec.path(view=completion)
    spec.path(view=completion_stream)
    spec.path(view=embedding)
